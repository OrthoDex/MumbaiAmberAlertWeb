class MessengerAlertBroadcastWorker
  ...
  def perform(url, name, reporter)
    ...
    subscriber_list.each do |subscriber|
      MessengerSuccessfulRegistrationNotifyWorker.perform_async(subscriber.to_i, "Amber Alert! #{name} was reported missing. If found, please contact #{reporter}. Find details here: #{url}")
    end
  end
end
